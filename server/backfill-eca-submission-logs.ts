...
async function main() {
  ...
  console.log(`ECA: ${targets.length} submissions with score>0 and submittedAt=null.`);
  ...
  for (const s of targets) {
    ...
    const moduleStart = moduleStartByAssignment.get(s.assignmentId); if (!moduleStart) { skipped++; continue; }
    // Anchor at the later of moduleStart / studentStart (defensive).
    const anchor = moduleStart >= u.start ? moduleStart : u.start;
    const rng = makeRng(hash(`eca:${s.studentId}:${s.assignmentId}`));
    ...
    plans.push({ id: s.id, submittedAt, label: u.label, title: titleById.get(s.assignmentId) ?? '?', sessionDate: anchor });
  }
  ...
  for (const p of plans.slice(0, 8)) {
    ...
    console.log(`  ${p.label.padEnd(26).slice(0, 26)} | anchor ${p.sessionDate.toISOString().slice(0, 10)} → submitted ${tp.y}-${pad(tp.m + 1)}-${pad(tp.d)} ${pad(tp.h)}:${pad(tp.min)} ET | ${p.title}`);
  }
