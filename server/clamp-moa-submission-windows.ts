...
import { getMoaModuleRun } from './utils/moa-schedule.js';
...
async function main() {
  ...
  for (const a of assignments) {
    ...
    if (a.title.endsWith(' - Final')) continue; // finals handled by backfill-moa-final-exam-times
  ...
  console.log(`MOA non-final submissions with submittedAt: ${subs.length}`);
  ...
  for (const s of subs) {
    ...
    const run = getMoaModuleRun(modName, u.start);
    ...
    // Out of window → clamp. Reuse same distribution as backfill-moa-submission-logs (within 14-day band after first session).
    const rng = makeRng(hash(`moa-clamp:${s.studentId}:${s.assignmentId}`));
