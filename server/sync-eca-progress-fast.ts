// ECA progress sync (single-track, explicit windows).
import 'dotenv/config';
import pg from 'pg';
import { ECA_WINDOWS, ecaDbModuleName } from './utils/eca-rotation.js';
...
(async () => {
  const t0 = Date.now();
  const courseRow = await pool.query<{ id: string }>(`SELECT id FROM courses WHERE code='ECA' LIMIT 1`);
  if (!courseRow.rows[0]) { console.log('ECA not found'); await pool.end(); return; }
  ...
  const resolveModule = (scheduleName: string) => {
    const dbName = ecaDbModuleName(scheduleName);
    return dbName ? (modByExactName.get(dbName) || null) : null;
  };

  type Window = { moduleId: string; start: Date; end: Date };
  const windows: Window[] = [];
  for (const w of ECA_WINDOWS) {
    const mod = resolveModule(w.module);
    if (!mod) continue;
    windows.push({ moduleId: mod.id, start: w.start, end: w.end });
  }
  const unresolved = ECA_WINDOWS.filter(w => !resolveModule(w.module)).map(w => w.module);
  ...
  console.log(`Total ECA enrollments with startDate: ${enrRes.rows.length}`);
  ...
  for (const e of targets) {
    ...
    for (const [moduleId, row] of rowMap) {
      ...
      const nexts = windows.filter(w => w.moduleId === moduleId && w.start > now).map(w => w.start.getTime());
      ...
      await client.query(
        `INSERT INTO student_progress (id, enrollment_id, batch_id, module_id, status, started_at, completed_at)
         ...
         ON CONFLICT (enrollment_id, module_id) DO UPDATE SET
           status = EXCLUDED.status, started_at = EXCLUDED.started_at, completed_at = EXCLUDED.completed_at`, params);
