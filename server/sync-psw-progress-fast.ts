// PSW progress sync (single-track, explicit windows; supports parallel modules).
import 'dotenv/config';
import pg from 'pg';
import { PSW_WINDOWS } from './utils/psw-rotation.js';
...
(async () => {
  const t0 = Date.now();
  const courseRow = await pool.query<{ id: string }>(`SELECT id FROM courses WHERE code='PSW' LIMIT 1`);
  if (!courseRow.rows[0]) { console.log('PSW not found'); await pool.end(); return; }
  ...
  console.log(`Total PSW enrollments with startDate: ${enrRes.rows.length}`);
  ...
  for (const e of targets) {
    ...
    for (const w of resolved) {
      ...
      if (w.end <= now) {
        ...
        // If multiple parallel modules are in progress today, prefer the latest-started.
