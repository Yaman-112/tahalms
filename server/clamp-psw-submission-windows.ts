...
async function main() {
  const course = await prisma.course.findUnique({
    where: { code: 'PSW' },
    include: { modules: { select: { id: true, name: true, startDate: true, position: true } } },
  });
  if (!course) { console.error('PSW not found'); process.exit(1); }

  // Build per-module window: [startDate, next-module.startDate OR startDate + 14d]. Min 14d length.
  const modsSorted = [...course.modules].filter(m => m.startDate).sort((a, b) => a.startDate!.getTime() - b.startDate!.getTime());
  const windowByModuleName = new Map<string, { first: Date; last: Date }>();
  for (let i = 0; i < modsSorted.length; i++) {
    const m = modsSorted[i];
    const next = modsSorted[i + 1];
    const defaultLast = new Date(m.startDate!.getTime() + 14 * 86400000);
    const nextLast = next ? new Date(next.startDate!.getTime() - 86400000) : defaultLast;
    const last = nextLast.getTime() - m.startDate!.getTime() >= 14 * 86400000 ? nextLast : defaultLast;
    windowByModuleName.set(m.name, { first: m.startDate!, last });
  }
  ...
  for (const a of assignments) {
    ...
    if (!m) continue;
    const w = windowByModuleName.get(m.name);
    if (w) windowByAssignment.set(a.id, { ...w, moduleName: m.name });
  }
  ...
  console.log(`PSW submissions with submittedAt: ${subs.length}`);
  ...
  for (const s of subs) {
    ...
    const rng = makeRng(hash(`psw-clamp:${s.studentId}:${s.assignmentId}`));
