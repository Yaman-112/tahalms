// AC progress sync. Each session has a 1-week filler module and a multi-week
// program module running in parallel; both are tracked in studentProgress.
// enrollment.currentModuleId is set to the program module of today's session.
...
(async () => {
  const t0 = Date.now();
  const courseRow = await pool.query<{ id: string }>(`SELECT id FROM courses WHERE code='AC' LIMIT 1`);
  if (!courseRow.rows[0]) { console.log('AC not found'); await pool.end(); return; }
  ...
  // Resolve a schedule name (e.g. "MS Excel") -> DB module row.
  const resolveModule = (scheduleName: string) => {
    const dbName = acDbModuleName(scheduleName);
    if (!dbName) return null;
    return modByExactName.get(dbName) || null;
  };

  // Pre-compute filler windows: each filler is exactly 1 week (next session start).
  // Pre-compute program windows: from first row's start until program changes.
  type Window = { moduleId: string; start: Date; end: Date };
  const fillerWindows: Window[] = [];
  for (let i = 0; i < AC_SESSIONS.length; i++) {
    const s = AC_SESSIONS[i];
    if (!s.filler) continue;
    if (/winter break/i.test(s.filler)) continue;
    const next = AC_SESSIONS[i + 1];
  ...
  let i = 0;
  while (i < AC_SESSIONS.length) {
    const s = AC_SESSIONS[i];
    if (/winter break/i.test(s.program)) { i++; continue; }
    let j = i + 1;
    while (j < AC_SESSIONS.length && AC_SESSIONS[j].program === s.program) j++;
    // Run i..j-1 share the same program; window ends at AC_SESSIONS[j].start (or +1 week if last)
    const end = j < AC_SESSIONS.length
      ? AC_SESSIONS[j].start
      : new Date(AC_SESSIONS[j - 1].start.getTime() + 7 * 86400000);
    const mod = resolveModule(s.program);
    if (mod) programWindows.push({ moduleId: mod.id, start: s.start, end });
    i = j;
  }

  // Check that every schedule name resolved.
  const unresolved = new Set<string>();
  for (const s of AC_SESSIONS) {
