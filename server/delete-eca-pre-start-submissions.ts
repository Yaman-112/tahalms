...
async function main() {
  ...
  const moduleStartForAssignment = new Map<string, Date>();
  for (const a of assignments) {
    const m = resolve(a.title);
    if (m?.startDate) moduleStartForAssignment.set(a.id, m.startDate);
  }
  ...
  console.log(`ECA: ${assignments.length} assignments, ${startByUser.size} students with startDate`);
  ...
  for (const s of allSubs) {
    const ms = moduleStartForAssignment.get(s.assignmentId);
    if (!ms) continue;
    const u = startByUser.get(s.studentId);
    if (!u) continue;
    // Compare by date only (ignore time-of-day) to avoid timezone artifacts.
  ...
  console.log(`\nDeleting ${idsToDelete.length} submissions…`);
  let deleted = 0;
