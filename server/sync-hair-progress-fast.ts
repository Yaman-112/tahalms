// Hairstyling (HAIR) progress sync.
import 'dotenv/config';
import pg from 'pg';
import { HAIR_WINDOWS, hairDbModuleName } from './utils/hair-rotation.js';

const APPLY = process.argv.includes('--apply');
const FORCE = process.argv.includes('--force');
const pool = new pg.Pool({ connectionString: 'postgresql://postgres:vTfaPkADzStasUDwfWnwGtWXZrndPNPL@monorail.proxy.rlwy.net:20759/railway', max: 4 });

(async () => {
  const t0 = Date.now();
  const courseRow = await pool.query<{ id: string }>(`SELECT id FROM courses WHERE code='HAIR' LIMIT 1`);
  if (!courseRow.rows[0]) { console.log('HAIR not found'); await pool.end(); return; }
  ...
  const resolveModule = (scheduleName: string) => {
    const dbName = hairDbModuleName(scheduleName);
    return dbName ? (modByExactName.get(dbName) || null) : null;
  };

  type ResolvedWindow = { moduleId: string; start: Date; end: Date };
  const resolved: ResolvedWindow[] = [];
  for (const w of HAIR_WINDOWS) {
    const mod = resolveModule(w.module);
    if (!mod) { console.log('  unresolved:', w.module); continue; }
    resolved.push({ moduleId: mod.id, start: w.start, end: w.end });
  }

  const enrRes = await pool.query<{ id: string; user_id: string; batch_code: string | null; start_date: Date | null; current_module_id: string | null }>(
    `SELECT id, user_id, batch_code, start_date, current_module_id FROM enrollments WHERE course_id = $1 AND start_date IS NOT NULL`, [courseId]);
  const existingProgress = await pool.query<{ enrollment_id: string; batch_id: string }>(
    `SELECT DISTINCT enrollment_id, batch_id FROM student_progress WHERE enrollment_id IN (SELECT id FROM enrollments WHERE course_id = $1)`, [courseId]);
  ...
  const batchesRes = await pool.query<{ id: string; batch_code: string }>(`SELECT id, batch_code FROM batches WHERE course_id = $1`, [courseId]);
  ...
  console.log(`Total HAIR enrollments with startDate: ${enrRes.rows.length}`);
  console.log(`To process: ${targets.length} ${FORCE ? '(force)' : '(remaining only)'}`);
  console.log(`Today (UTC): ${now.toISOString().slice(0, 10)}`);
  console.log(`Mode: ${APPLY ? 'APPLY' : 'DRY-RUN'}`);

  type Row = { enrollmentId: string; moduleId: string; batchId: string; status: 'COMPLETED'|'IN_PROGRESS'|'NOT_STARTED'; startedAt: Date | null; completedAt: Date | null };
  const allRows: Row[] = [];
  ...
  for (const e of targets) {
    ...
    let latestCurrentStart = -Infinity;
    for (const w of resolved) {
      ...
      if (w.end <= now) {
        ...
        if (w.start.getTime() > latestCurrentStart) {
          currentModuleId = w.moduleId;
          latestCurrentStart = w.start.getTime();
        }
      }
    }
    for (const [moduleId, row] of rowMap) {
      if (row.status !== 'NOT_STARTED') continue;
      const nexts = resolved.filter(w => w.moduleId === moduleId && w.start > now).map(w => w.start.getTime());
  ...
  for (const [, mid] of currentByEnrollment) {
    ...
    byCurrent[m || '(none)'] = (byCurrent[m || '(none)'] ?? 0) + 1;
  }
  console.log('Current today:');
  for (const [k, v] of Object.entries(byCurrent).sort((a, b) => b[1] - a[1])) console.log(`  ${k.padEnd(50)} ${v}`);
  ...
  try {
    ...
    for (let k = 0; k < allRows.length; k += CHUNK) {
      ...
      await client.query(`INSERT INTO student_progress (id, enrollment_id, batch_id, module_id, status, started_at, completed_at) VALUES ${values.join(',')}
         ON CONFLICT (enrollment_id, module_id) DO UPDATE SET status = EXCLUDED.status, started_at = EXCLUDED.started_at, completed_at = EXCLUDED.completed_at`, params);
      console.log(`  upserted ${Math.min(k + CHUNK, allRows.length)} / ${allRows.length}`);
    }
    ...
    for (let k = 0; k < enrEntries.length; k += CHUNK) {
      ...
      for (const [eid, mid] of batch) {
        ids.push(`$${p++}`); params.push(eid);
        if (mid === null) cases.push(`WHEN id = $${p - 1} THEN NULL`);
        else { cases.push(`WHEN id = $${p - 1} THEN $${p++}::text`); params.push(mid); }
      }
      await client.query(`UPDATE enrollments SET current_module_id = CASE ${cases.join(' ')} END WHERE id IN (${ids.join(',')})`, params);
      console.log(`  enrollments updated ${Math.min(k + CHUNK, enrEntries.length)} / ${enrEntries.length}`);
    }
