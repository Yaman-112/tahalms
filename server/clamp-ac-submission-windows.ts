...
import { getAcModuleRun } from './utils/ac-schedule.js';
...
async function main() {
  ...
  for (const a of assignments) {
    ...
    if (a.title.endsWith(' - Final')) continue; // finals handled by backfill-ac-final-exam-times
    const m = resolve(a.title);
    if (m) moduleNameForAssignment.set(a.id, m.name);
  }
  ...
  for (const e of enrollments) {
    ...
    if (prev && e.startDate! >= prev.start) continue;
    startByUser.set(e.userId, { start: e.startDate!, label: `${e.user.firstName} ${e.user.lastName}` });
  }

  const subs = await prisma.submission.findMany({
    where: { assignmentId: { in: Array.from(moduleNameForAssignment.keys()) }, submittedAt: { not: null } },
    select: { id: true, studentId: true, assignmentId: true, submittedAt: true },
  });
  console.log(`AC non-final submissions with submittedAt: ${subs.length}`);
  ...
  for (const s of subs) {
    ...
    const run = getAcModuleRun(modName, u.start);
    if (!run) { skipped++; continue; }

    // Window: [first session 00:00 ET, last session + 7 days 23:59 ET]
    const winStart = run.first;
    const winEnd = new Date(run.last.getTime() + 7 * 86400000);
    ...
    // Out of window → clamp. Reuse same distribution as backfill-ac-submission-logs (within 14-day band after first session).
    const rng = makeRng(hash(`ac-clamp:${s.studentId}:${s.assignmentId}`));
    const spanDays = Math.max(7, Math.round((winEnd.getTime() - winStart.getTime()) / 86400000));
    const frac = rng() < 0.7 ? 0.55 + rng() * 0.45 : rng() * 0.9;
    // Use noon UTC as the base to avoid ET date-shifting at the window edges.
    ...
    const newAt = torontoWallToUtc(tp.y, tp.m, tp.d, hour, minute, second);
    plans.push({ id: s.id, oldAt: at, newAt, label: u.label, title: titleById.get(s.assignmentId) ?? '?', first: run.first, last: run.last });
  }
