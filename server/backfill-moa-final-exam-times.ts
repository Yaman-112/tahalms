...
async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'MOA' }, include: { modules: { select: { name: true } } } });
  ...
  console.log(`MOA "- Final" assignments: ${finals.length}`);
  ...
  for (const s of subs) {
    ...
    const run = getMoaModuleRun(meta.moduleName, u.startDate);
    if (!run) { skipped++; continue; }
    // Exam day = last session (a Monday) + 3 days (Thursday of that week)
    const examDay = new Date(run.last.getTime() + 3 * 86400000);

    const rng = makeRng(hash(`moa-final-exam:${s.studentId}:${s.assignmentId}`));
