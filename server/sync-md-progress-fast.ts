// MD progress sync (single-track, explicit windows; final 5 modules parallel).
import 'dotenv/config';
import pg from 'pg';
import { MD_WINDOWS } from './utils/md-rotation.js';
...
(async () => {
  const t0 = Date.now();
  const courseRow = await pool.query<{ id: string }>(`SELECT id FROM courses WHERE code='MD' LIMIT 1`);
  if (!courseRow.rows[0]) { console.log('MD not found'); await pool.end(); return; }
  ...
  for (const w of MD_WINDOWS) {
    const mod = modByName.get(norm(w.module));
  ...
  console.log(`Total MD enrollments with startDate: ${enrRes.rows.length}`);
  ...
  for (const [k, v] of Object.entries(byCurrent).sort((a, b) => b[1] - a[1])) console.log(`  ${k.padEnd(60)} ${v}`);
