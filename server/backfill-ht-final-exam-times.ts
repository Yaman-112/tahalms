...
function parseShiftStart(shift: string | null, classTime: string | null): { h: number; m: number } {
  ...
  if (sl.includes('weekend')) return { h: 9, m: 0 };
  return { h: 9, m: 30 };
}

async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'HT' }, include: { modules: { select: { name: true } } } });
  if (!course) { console.error('HT not found'); process.exit(1); }
  ...
  console.log(`HT "- Final" assignments: ${finals.length}`);
  ...
  type UserInfo = { classDays: string | null; track: 'weekday' | 'weekend'; batchCode: string | null; label: string; start: { h: number; m: number } };
  ...
  for (const e of enrollments) {
    ...
    infoByUser.set(e.userId, { classDays: e.classDays, track, batchCode: e.batchCode, label: `${e.user.firstName} ${e.user.lastName}`, start });
  }
  ...
  for (const s of subs) {
    ...
    // HT: weekday track has 2 weekly sessions per module (Monday of wk1, Monday of wk2). Exam = wk2 Thursday (+3).
    // Weekend has sessions on Fri/Sat/Sun (one module per weekend block typically).
    const sessionDate = getHtFirstSessionDate(meta.moduleName, u.track);
    if (!sessionDate) { skipped++; continue; }
    // Exam day = session start + (7 days week-2) + (track-specific day offset)
    const week2 = new Date(sessionDate.getTime() + 7 * 86400000);
    const offsetDays = u.track === 'weekend' ? 2 : 3;
    const examDay = new Date(week2.getTime() + offsetDays * 86400000);

    const rng = makeRng(hash(`ht-final-exam:${s.studentId}:${s.assignmentId}`));
