...
async function main() {
  ...
  console.log(`HAIR "- Final" assignments: ${finals.length}`);
  type FinalMeta = { moduleName: string; title: string; durationMin: number; examDay: Date };
  const assignmentMeta = new Map<string, FinalMeta>();
  for (const a of finals) {
    const mod = resolve(a.title);
    if (!mod?.startDate) continue;
    let dur = a.timeLimit ?? 0;
    if (!dur) {
      if (a.format === 'MCQ' || a.format === 'MIXED') dur = Math.max(30, Math.min(90, a._count.questions * 1.5));
      else dur = 45;
    }
    // Exam = last session + 3 days (Thursday of final week, assuming Monday sessions).
    const weeks = moduleWeeks(mod.hours);
    const lastSession = new Date(mod.startDate.getTime() + (weeks - 1) * 7 * 86400000);
    const examDay = new Date(lastSession.getTime() + 3 * 86400000);
    assignmentMeta.set(a.id, { moduleName: mod.name, title: a.title, durationMin: dur, examDay });
  }
  ...
  type UserInfo = { batchCode: string | null; label: string; start: { h: number; m: number } };
  const infoByUser = new Map<string, UserInfo>();
  for (const e of enrollments) {
    if (infoByUser.has(e.userId)) continue;
    infoByUser.set(e.userId, { batchCode: e.batchCode, label: `${e.user.firstName} ${e.user.lastName}`, start: parseShiftStart(e.user.shift, e.classTime) });
  }
  ...
  for (const s of subs) {
    ...
    const examDay = meta.examDay;
    const rng = makeRng(hash(`hair-final-exam:${s.studentId}:${s.assignmentId}`));
