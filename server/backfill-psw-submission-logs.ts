...
async function main() {
  const course = await prisma.course.findUnique({ where: { code: 'PSW' }, include: { modules: { select: { name: true, startDate: true } } } });
  ...
  console.log(`PSW: ${targets.length} submissions with score>0 and submittedAt=null.`);
  ...
  for (const s of targets) {
    ...
    const ms = moduleStartByAssignment.get(s.assignmentId); if (!ms) { skipped++; continue; }
    const anchor = ms >= u.start ? ms : u.start;
    const rng = makeRng(hash(`psw:${s.studentId}:${s.assignmentId}`));
  ...
  for (const p of plans.slice(0, 6)) {
    ...
    console.log(`  ${p.label.padEnd(26).slice(0, 26)} | anchor ${p.anchor.toISOString().slice(0, 10)} → submitted ${tp.y}-${pad(tp.m + 1)}-${pad(tp.d)} ${pad(tp.h)}:${pad(tp.min)} ET | ${p.title}`);
  }
