...
import { getCswFirstSessionDateForStudent, normalizeCswModuleName } from './utils/csw-schedule.js';
...
function torontoWallToUtc(y: number, m: number, d: number, h: number, min: number, s: number): Date {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const wallGuess = Date.UTC(p.y, p.m, p.d, p.h, p.min, p.s);
    const desired = Date.UTC(y, m, d, h, min, s);
    const diff = desired - wallGuess;
    if (diff === 0) break;
...
async function main() {
  ...
  console.log(`CSW: ${targets.length} submissions with score > 0 and submittedAt=null.`);
  console.log(`Mode: ${WRITE ? 'WRITE' : 'DRY RUN'}\n`);

  type Plan = { id: string; submittedAt: Date; label: string; title: string; sessionDate: Date };
  const plans: Plan[] = [];
  const assignTitleById = new Map(assignments.map(a => [a.id, a.title]));
  let skipped = 0;
  ...
  for (const s of targets) {
    ...
    const sessionDate = getCswFirstSessionDateForStudent(modName, u.start);
    if (!sessionDate) { skipped++; continue; }

    const rng = makeRng(hash(`csw:${s.studentId}:${s.assignmentId}`));
    const windowDays = 14;
    const frac = rng() < 0.7 ? 0.55 + rng() * 0.45 : rng() * 0.9;
    const baseMs = sessionDate.getTime() + windowDays * 86400000 * frac;
    const r = rng();
    ...
    plans.push({ id: s.id, submittedAt, label: u.label, title: assignTitleById.get(s.assignmentId) ?? '?', sessionDate });
  }
  ...
  if (!WRITE) { console.log('\nDRY RUN — re-run with --write to commit.'); await pool.end(); return; }
